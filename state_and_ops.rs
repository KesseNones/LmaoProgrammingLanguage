...
impl Operators{
	pub fn new() -> Self{
		let pairs: Vec<(&str, OpFunction)> = vec![
			("+", add),
			("-", sub),
			("*", mult),
			("/", div),
			...
			("changeItemAt", change_item_at),

			("isWhitespaceChar", whitespace_detect),
			("isAlphaChar", alpha_char_detect),
			("isNumChar", num_char_detect),

			("objAddField", add_field),
			("objGetField", get_field),
			("objMutField", mut_field),
			("objRemField", remove_field),
			...
			("leaveScopeIfTrue", leave_scope_if_true),
			("throwCustomError", throw_custom_error),
			("getArgs", get_args),
			("isValidBox", is_valid_box),
			("timeUnixNow", time_unix_now),
			("timeWait", time_wait),
		];

		let name_to_func_hash: HashMap<String, OpFunction> = pairs
			.into_iter()		
			.map(|(name, func)| (name.to_string(), func))
			.collect();
		let ops_map = make_ops_hashmap();
			
		let mut op_pairs: Vec<(&String, &usize)> = ops_map.iter().collect();
		op_pairs.sort_by_key(|&(key, val)| val);
	
		let mut funcs: Vec<OpFunction> = Vec::new();
	
		for (key, _) in op_pairs{
			if let Some(func) = name_to_func_hash.get(key){
				funcs.push(*func);	
			}
		}		

		Operators{
			ops: funcs.into_boxed_slice(),
			cast_index: *ops_map.get("cast").unwrap()
		}	
	
	}

	pub fn run_op(&self, num: usize, s: &mut Stack, h: &mut Heap, args: Option<&str>) -> 
	Option<Result<RetCode, String>>
	{
		if num == 0 {return None;}

		if let Some(op) = self.ops.get(num - 1){
			Some(op(s, h, args))
		}else {None}
	}

	pub fn cast_op_index(&self) -> usize {self.cast_index}
